...
export class BlazeService {
  ...
  private static async fetchBlazeAPI(): Promise<BlazeHistoryEntry[]> {
    ...
    try {
      const supabaseFunctionUrl = 'https://bmrhhjsduebxtsguwuts.supabase.co/functions/v1/blaze-history';
      ...
      return games.slice(0, this.HISTORY_LIMIT).map((game: BlazeGame, index: number) => {
        const number = game.roll ?? game.value ?? game.number ?? game.result ?? 0;
        return {
          id: game.id?.toString() || `game_${Date.now()}_${index}`,
          number,
          color: this.getNumberColor(number),
          timestamp: new Date(game.created_at || game.date || game.timestamp || Date.now() - index * 30000).getTime()
        };
      });
      ...
      console.error(`❌ Erro na tentativa ${this.retryCount}/${this.MAX_RETRIES}: ${errorMessage}`);
  ...
  static async getHistory(): Promise<BlazeHistoryEntry[]> {
    ...
    this.fetchPromise = (async () => {
      try {
        console.log('🔍 Cache expirado ou inexistente, buscando novos dados...');
  ...
  static async getStoneStats(): Promise<StoneStats[]> {
    ...
    for (let stone = 0; stone <= 14; stone++) {
      ...
      const percentage = totalGames > 0 ? (occurrences / totalGames) * 100 : 0;

      let redCount = 0;
      let blackCount = 0;

      stoneOccurrences.forEach(occurrence => {
        const currentIndex = history.findIndex(h => h.id === occurrence.id);
      ...
      const redTendency = totalNext > 0 ? (redCount / totalNext) * 100 : 50;
      const blackTendency = totalNext > 0 ? (blackCount / totalNext) * 100 : 50;
  ...
  static async getColorStats(): Promise<ColorStats> {
    ...
    const redCount = recentHistory.filter(entry => entry.color === 'red').length;
    const blackCount = recentHistory.filter(entry => entry.color === 'black').length;
    const whiteCount = recentHistory.filter(entry => entry.color === 'white').length;

    return {
      redPercentage: totalGames > 0 ? Math.round((redCount / totalGames) * 100) : 0,
      blackPercentage: totalGames > 0 ? Math.round((blackCount / totalGames) * 100) : 0,
      whitePercentage: totalGames > 0 ? Math.round((whiteCount / totalGames) * 100) : 0,
      total: totalGames
    };
  }
